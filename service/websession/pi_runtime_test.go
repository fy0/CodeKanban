...
func TestManagerPiRPCTreeForkAndCloneCreateIndependentSessions(t *testing.T) {
	for _, testCase := range []struct {
		name      string
		operation string
	}{
		{name: "fork", operation: "fork"},
		...
		t.Run(testCase.name, func(t *testing.T) {
			...
			project := seedProject(t)
			fakeDir := t.TempDir()
			sourcePath := filepath.Join(fakeDir, testCase.name+"-source.jsonl")
			...
			userID := ""
			for _, node := range tree.Nodes {
				if node.Preview == "pr6-tree-seed" {
					userID = node.ID
				}
			}
			if userID == "" {
				t.Fatalf("missing forkable user node: %#v", tree)
			}
			...
			var result PiTreeCreateResult
			if testCase.operation == "fork" {
				result, err = manager.ForkPiSessionTree(context.Background(), created.ID, PiTreeForkInput{
					TargetID: userID, Revision: tree.Revision,
				})
			} else {
				result, err = manager.ClonePiSessionTree(context.Background(), created.ID, PiTreeCloneInput{
					Revision: tree.Revision,
				})
			}
			if err != nil {
				t.Fatalf("%s Pi tree: %v", testCase.operation, err)
			}
			if result.Session.ID == "" || result.Session.ID == created.ID || result.Session.Agent != AgentPi {
				t.Fatalf("unexpected target session: %#v", result.Session)
			}
			target := mustGetSession(t, manager, result.Session.ID)
			if pointerString(target.NativeSessionID) == "" || pointerString(target.NativeSessionID) == pointerString(sourceBefore.NativeSessionID) ||
				pointerString(target.ThreadPath) == "" || samePiRuntimePath(pointerString(target.ThreadPath), pointerString(sourceBefore.ThreadPath)) {
				t.Fatalf("tree mutation did not create an independent native identity: source=%#v target=%#v", sourceBefore, target)
			}
			if target.Backend != string(SessionBackendPiRPC) || target.Model != sourceBefore.Model || target.ReasoningEffort != sourceBefore.ReasoningEffort ||
				target.TotalInputTokens != 10 || target.TotalCachedInputTokens != 2 || target.TotalOutputTokens != 5 || target.SessionContextWindowTokens != 32000 {
				t.Fatalf("target config/usage was not projected atomically: %#v", target)
			}
			...
			if pointerString(sourceAfter.NativeSessionID) != pointerString(sourceBefore.NativeSessionID) ||
				...
				t.Fatalf("tree mutation changed source identity: before=%#v after=%#v", sourceBefore, sourceAfter)
			}
			historyAfter, err := manager.History(context.Background(), created.ID, DefaultHistoryWindow, nil)
			if err != nil {
				t.Fatal(err)
			}
			if len(historyAfter.Items) != len(historyBefore.Items) || !historyContainsText(historyAfter, "active branch") || historyContainsText(historyAfter, "abandoned branch") {
				t.Fatalf("tree mutation changed source history: before=%#v after=%#v", historyBefore.Items, historyAfter.Items)
			}

			targetHistory, err := manager.History(context.Background(), result.Session.ID, DefaultHistoryWindow, nil)
			if err != nil {
				t.Fatal(err)
			}
			if testCase.operation == "fork" {
				if result.EditorText != "pr6-tree-seed" || len(targetHistory.Items) != 0 || result.Tree.LeafID != nil {
					t.Fatalf("unexpected root fork result: result=%#v history=%#v", result, targetHistory.Items)
				}
			} else if result.EditorText != "" || !historyContainsText(targetHistory, "pr6-tree-seed") ||
				!historyContainsText(targetHistory, "active branch") || historyContainsText(targetHistory, "abandoned branch") {
				t.Fatalf("unexpected clone projection: result=%#v history=%#v", result, targetHistory.Items)
			}

			manager.piRuntimeMu.Lock()
			_, sourceRuntimePresent := manager.piRuntimes[created.ID]
			_, targetRuntimePresent := manager.piRuntimes[result.Session.ID]
			manager.piRuntimeMu.Unlock()
			if sourceRuntimePresent || targetRuntimePresent {
				t.Fatalf("mutated Pi runtime was retained: source=%v target=%v", sourceRuntimePresent, targetRuntimePresent)
			}
			if _, err := manager.GetPiSessionTree(context.Background(), created.ID); err != nil {
				t.Fatalf("restore source tree: %v", err)
			}
			if _, err := manager.GetPiSessionTree(context.Background(), result.Session.ID); err != nil {
				t.Fatalf("restore target tree: %v", err)
			}
			...
			for _, command := range readFakePiLog(t, logPath) {
				...
				for index := 0; index+1 < len(args); index++ {
					if args[index] == "--session" {
						restoredPaths[filepath.Clean(stringValue(args[index+1]))] = true
					}
				}
			}
			if !restoredPaths[filepath.Clean(sourcePath)] || !restoredPaths[filepath.Clean(pointerString(target.ThreadPath))] {
				t.Fatalf("source/target were not restored from independent files: %#v", restoredPaths)
			}
		})
	}
}
