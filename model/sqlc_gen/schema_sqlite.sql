...
CREATE TABLE "web_session_items" ("id" text NOT NULL,"created_at" datetime,"updated_at" datetime,"deleted_at" datetime,"web_session_id" text NOT NULL,"web_turn_id" text,"source_thread_id" text,"source_turn_id" text,"source_item_id" text,"command_group_id" text,"order_index" integer NOT NULL,"last_event_seq" integer NOT NULL DEFAULT 0,"run_id" text,"run_duration_ms" integer,"run_outcome" text,"item_kind" text NOT NULL,"item_type" text NOT NULL,"role" text,"status" text,"level" text,"text" text,"done" boolean NOT NULL DEFAULT false,"timestamp" datetime,"observed_at" datetime,"attachments_json" text,"tool_json" text,"detail_json" text,"payload_json" text,PRIMARY KEY ("id"));
...
CREATE INDEX "idx_web_session_item_event" ON "web_session_items"("web_session_id","last_event_seq","order_index");
